...
class Frame(object):
    ...
    if not _logger.handlers:  # pylint: disable=R0801
        _logger.setLevel(direwolf.LOG_LEVEL)  # pylint: disable=R0801
        _console_handler = logging.StreamHandler()  # pylint: disable=R0801
        _console_handler.setLevel(direwolf.LOG_LEVEL)  # pylint: disable=R0801
        _console_handler.setFormatter(direwolf.LOG_FORMAT)  # pylint: disable=R0801
    ...
    def __init__(self, source: bytes = b'', destination: bytes = b'',
        ...
        self.source = direwolf.parse_callsign(source)
        self.destination = direwolf.parse_callsign(destination)
        # TODO: Add parse_path function
        self.path = path
        self.info = direwolf.parse_info_field(info)
    ...
    def set_path(self, path=[]) -> None:
        self.path = [direwolf.parse_callsign(pth) for pth in path]

    def update_path(self, update: bytes) -> None:
        self.path.append(direwolf.parse_callsign(update))
    ...
    def encode_ax25(self) -> bytes:
        ...
        """
        encoded_frame = []
        encoded_frame.append(direwolf.AX25_FLAG)
        ...
        encoded_frame.append(direwolf.ADDR_INFO_DELIM)
...
class APRS(object):
    ...
    def receive(self, callback=None, frame_handler=direwolf.parse_frame):
        """
...
class TCP(APRS):
    ...
    def __init__(self, user: bytes, password: bytes, servers: bytes = b'',
        ...
        servers = servers or direwolf.APRSIS_SERVERS  # Unicode
    ...
    def start(self):
        ...
        while not self._connected:
            ...
            if b':' in servers:
                ...
                server = servers
                port = direwolf.APRSIS_FILTER_PORT
    ...
    def receive(self, callback=None, frame_handler=direwolf.parse_frame):
        ...
        try:
            while 1:
                recv_data = self.interface.recv(direwolf.RECV_BUFFER)
...
class UDP(APRS):
    ...
    def __init__(self, user, password='-1', server=None, port=None):
        ...
        server = server or direwolf.APRSIS_SERVERS[0]
        port = port or direwolf.APRSIS_RX_PORT
...
class HTTP(APRS):
    ...
    def __init__(self, user: bytes, password: bytes = b'-1', url: bytes = b'',
        ...
        self.url = url or direwolf.APRSIS_URL
        self.headers = headers or direwolf.APRSIS_HTTP_HEADERS
        self.use_i_construct = True
    ...
    def send(self, frame: bytes) -> bool:
        ...
        if isinstance(frame, str):
            frame = direwolf.parse_frame(frame)
        if isinstance(frame, direwolf.Frame):
            frame = bytes(frame)
        self._logger.info('Sending frame="%s"', frame)
...
class PositionFrame(Frame):
    ...
    def create_info_field(self) -> bytes:
        enc_lat = direwolf.dec2dm_lat(self.lat)
        enc_lat_amb = bytes(direwolf.ambiguate(enc_lat, self.ambiguity), 'UTF-8')
        enc_lng = direwolf.dec2dm_lng(self.lng)
        enc_lng_amb = bytes(direwolf.ambiguate(enc_lng, self.ambiguity), 'UTF-8')
        frame = [
            b'=',
            enc_lat_amb,
            self.table,
