...
class Frame(object):
    ...
    def parse(self, frame=None):
        """
        Parses a Frame from either ASCII or KISS Encoded frame.
        """
    ...
    def parse_text(self):
        ...
        for char in self.frame:
            if '>' in char and not self.source:
                self.source = direwolf.Callsign(frame_so_far)
                frame_so_far = ''
            elif ':' in char:
                if not self.path:
                    if ',' in frame_so_far:
                        self.destination = direwolf.Callsign(
                            frame_so_far.split(',')[0])
                        self.path = []
                        for path in frame_so_far.split(',')[1:]:
                            self.path.append(direwolf.Callsign(path))
                        frame_so_far = ''
                    elif not self.destination:
                        self.destination = direwolf.Callsign(frame_so_far)
                        frame_so_far = ''
                    else:
    ...
    def parse_kiss(self):
        """
        Parses and Extracts the components of a KISS-Encoded Frame.
        """
    ...
    def _extract_kiss_path(self, start):
        ...
        for i in range(2, start):
            path_call = direwolf.Callsign(self.frame[i * 7:])
