...
def parse_frame(raw_frame: typing.Union[bytes, str]) -> AprsFrame:
    ...
    """
    if isinstance(raw_frame, direwolf.Frame):
        return raw_frame
        ...
        if direwolf.ADDR_INFO_DELIM in raw_frame:
...
def parse_frame_ax25(raw_frame: bytes) -> AprsFrame:
    ...
    _frame = raw_frame.strip(direwolf.AX25_FLAG)
    if (_frame.startswith(direwolf.KISS_DATA_FRAME) or
            _frame.endswith(direwolf.KISS_DATA_FRAME)):
        _frame = _frame.lstrip(direwolf.KISS_DATA_FRAME)
        _frame = _frame.rstrip(direwolf.KISS_DATA_FRAME)
        kiss_call = True

    # Use these two fields as the address/information delimiter
    frame_addressing, frame_information = _frame.split(direwolf.ADDR_INFO_DELIM)
...
def parse_callsign(raw_callsign: bytes) -> AprsCallsign:
    ...
    """
    if isinstance(raw_callsign, direwolf.Callsign):
        return raw_callsign
    try:
...
def parse_callsign_text(raw_callsign: bytes) -> AprsCallsign:
    ...
    """
    parsed_callsign: AprsCallsign = direwolf.Callsign()
...
def parse_callsign_ax25(raw_callsign: bytes, kiss_call: bool = False) -> AprsCallsign:
    ...
    :type frame: str
    """
    parsed_callsign = direwolf.Callsign()
    _callsign = bytes()
    digi = False
    ...
    for _chunk in raw_callsign[:6]:
        ...
        if chunk & 1:
            ...
            raise direwolf.BadCallsignError('Bad address-end flag.')
...
def parse_info_field(raw_data: bytes, handler=None) -> bytes:
    if not raw_data:
        return bytes()
    elif isinstance(raw_data, direwolf.InformationField):
        return raw_data
    elif isinstance(raw_data, bytes) or isinstance(raw_data, bytearray):
        data_type = b''
        data_type_field = chr(raw_data[0])
        data_type = direwolf.DATA_TYPE_MAP.get(data_type_field)
        ...
        return direwolf.InformationField(raw_data, data_type, safe=True)
...
def default_data_handler(data: bytes, data_type: bytes) -> bytes:
    ...
    return direwolf.InformationField(decoded_data, data_type)
