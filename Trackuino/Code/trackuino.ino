...
void setup()
{
  ...
  if (APRS_SLOT >= 0) {
    ...
    //     Serial.write("Still waiting");
    //     power_save();
    // } while (! gps_decode(Serial.read()));
    next_aprs = millis() + 1000 *
      (APRS_PERIOD - (APRS_PERIOD - APRS_SLOT) % APRS_PERIOD);
  }
  else {
    next_aprs = millis();
  }  
...
void loop()
{
  ...
  if ((int32_t) (millis() - next_aprs) >= 0) {
    ...
    afsk_debug();
#endif
//commented out because it sounds like it only relates to GPS (yeah i'm lazy)
  // } else {
  ...
  //power_save(); // Incoming GPS data or interrupts will wake us up| commented out because it had the word GPS in it
}
}
